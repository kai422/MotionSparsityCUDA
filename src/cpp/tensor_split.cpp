 ...
 * @ Modified by: Kai Xu
 * @ Modified time: 2020-06-10 14:28:58
 * @ Description: split dense tensor to three sparse tensors with hierarchy of
 * different depths.
 */
...
void DenseSplitForwardCPU(const torch::Tensor input, torch::Tensor out_l1,
                          torch::Tensor out_l2, torch::Tensor out_l3,
                          torch::Tensor out_l4,
                          ptr_wrapper<quadtree *> structures) {
  // please make sure out_l_r are zero tensor.

  auto dim = input.ndimension();

  TORCH_CHECK(dim == 4,
              "MotionSparsityError: expected 3D tensor, but got tensor with ",
              dim, " dimensions instead");
  TORCH_CHECK(
      input.sizes() == out_l1.sizes(),
      "MotionSparsityError: expected dst and src tensors have the same shape");
  TORCH_CHECK(
      input.sizes() == out_l2.sizes(),
      "MotionSparsityError: expected dst and src tensors have the same shape");
  TORCH_CHECK(
      input.sizes() == out_l3.sizes(),
      "MotionSparsityError: expected dst and src tensors have the same shape");
  TORCH_CHECK(
      input.sizes() == out_l4.sizes(),
      "MotionSparsityError: expected dst and src tensors have the same shape");
  ...
  for (auto t = 0; t < T; t++) {
    ...
    splitQuadToDense(f, h, w, stru_t, out_l1_t, out_l2_t, out_l3_t, out_l4_t,
                     border_coords_l1, border_coords_l2, border_coords_l3,
                     border_coords_l4);
...
void splitQuadToDense(const int &f, const int &tensor_h, const int &tensor_w,
                      quadtree *input_quad, torch::Tensor out_l1_dst,
                      torch::Tensor out_l2_dst, torch::Tensor out_l3_dst,
                      torch::Tensor out_l4_dst,
                      std::vector<std::tuple<int, int>> &border_coords_l1,
                      std::vector<std::tuple<int, int>> &border_coords_l2,
                      std::vector<std::tuple<int, int>> &border_coords_l3,
                      std::vector<std::tuple<int, int>> &border_coords_l4) {
  ...
  assert(f == feature_size &&
         ((float)tensor_h / input_quad->grid_height) ==
             ((float)input_quad->grid_width / tensor_w) &&
  ...
  for (int grid_idx = 0; grid_idx < n_blocks; ++grid_idx) {
    ...
    qt_data_t *grid_data =
        input_quad->data +
        input_quad->feature_size * input_quad->prefix_leafs[grid_idx];
    ...
    if (tree_isset_bit(grid_tree, 0)) {
      for (int hl1 = 0; hl1 < 2; ++hl1) {
        for (int wl1 = 0; wl1 < 2; ++wl1) {
          ...
          if (tree_isset_bit(grid_tree, bit_idx_l1)) {
                ...
                if (tree_isset_bit(grid_tree, bit_idx_l2)) {
                  for (int hl3 = 0; hl3 < 2; ++hl3) {
                    for (int wl3 = 0; wl3 < 2; ++wl3) {
                      ...
                      int data_idx =
                          tree_data_idx(grid_tree, bit_idx_l3, feature_size);
                      save_data_to_tensor_with_border_coords(
                          grid_data + data_idx, out_l4_dst, scale_factor,
                          feature_size, centre_x_l3 - 0.5, centre_x_l3 + 0.5,
                          centre_y_l3 - 0.5, centre_y_l3 + 0.5,
                          border_coords_l4);
                    }
                  }
                } else {
                  int data_idx =
                      tree_data_idx(grid_tree, bit_idx_l2, feature_size);
                  save_data_to_tensor_with_border_coords(
                      grid_data + data_idx, out_l3_dst, scale_factor,
                      feature_size, centre_x_l2 - 1, centre_x_l2 + 1,
                      centre_y_l2 - 1, centre_y_l2 + 1, border_coords_l3);
                }
              }
            }
          } else {
            ...
            save_data_to_tensor_with_border_coords(
                grid_data + data_idx, out_l2_dst, scale_factor, feature_size,
                centre_x_l1 - 2, centre_x_l1 + 2, centre_y_l1 - 2,
                centre_y_l1 + 2, border_coords_l2);
          }
        }
      }
    } else {
      ...
      save_data_to_tensor_with_border_coords(
          grid_data, out_l1_dst, scale_factor, feature_size, centre_x - 4,
          centre_x + 4, centre_y - 4, centre_y + 4, border_coords_l1);
    }
  }
}

void get_padded_tensor(torch::Tensor input_tensor, torch::Tensor ref,
                       std::vector<std::tuple<int, int>> &border_coords) {
...
void DenseSplitBackwardCPU(torch::Tensor grad_in, torch::Tensor grad_out_l1,
                           torch::Tensor grad_out_l2, torch::Tensor grad_out_l3,
                           torch::Tensor grad_out_l4,
                           ptr_wrapper<quadtree *> structures) {
  ...
  TORCH_CHECK(dim == 4,
              ...
              dim, " dimensions instead");
  TORCH_CHECK(
      grad_in.sizes() == grad_out_l1.sizes(),
      "MotionSparsityError: expected dst and src tensors have the same shape");
  TORCH_CHECK(
      grad_in.sizes() == grad_out_l2.sizes(),
      "MotionSparsityError: expected dst and src tensors have the same shape");
  TORCH_CHECK(
      grad_in.sizes() == grad_out_l3.sizes(),
      "MotionSparsityError: expected dst and src tensors have the same shape");
  TORCH_CHECK(
      grad_in.sizes() == grad_out_l4.sizes(),
      "MotionSparsityError: expected dst and src tensors have the same shape");
  ...
  for (auto t = 0; t < T; t++) {
    ...
    assert(f == stru_t->feature_size &&
           ((float)h / stru_t->grid_height) ==
               ((float)stru_t->grid_width / w) &&
           "expect input structure has same size with data tensor.");
    _unused(f);
    _unused(w);
...
#pragma omp parallel for num_threads(16)
    for (int grid_idx = 0; grid_idx < n_blocks; ++grid_idx) {
      ...
      if (tree_isset_bit(grid_tree, 0)) {
                  ...
                  if (tree_isset_bit(grid_tree, bit_idx_l2)) {
                    for (int hl3 = 0; hl3 < 2; ++hl3) {
                      for (int wl3 = 0; wl3 < 2; ++wl3) {
                        ...
                        float centre_x_l3 = centre_x_l2 + (wl3 * 1) - 0.5;
                        float centre_y_l3 = centre_y_l2 + (hl3 * 1) - 0.5;
                        assign_data_among_tensor(
                            grad_in_t, grad_out_l4_t, scale_factor,
                            feature_size, centre_x_l3 - 0.5, centre_x_l3 + 0.5,
                            centre_y_l3 - 0.5, centre_y_l3 + 0.5);
                      }
                    }
                  } else {
                    assign_data_among_tensor(grad_in_t, grad_out_l3_t,
                                             scale_factor, feature_size,
                                             centre_x_l2 - 1, centre_x_l2 + 1,
                                             centre_y_l2 - 1, centre_y_l2 + 1);
                  }
                }
              }
            } else {
              assign_data_among_tensor(grad_in_t, grad_out_l2_t, scale_factor,
                                       feature_size, centre_x_l1 - 2,
                                       centre_x_l1 + 2, centre_y_l1 - 2,
                                       centre_y_l1 + 2);
            }
          }
        }
        ...
        assign_data_among_tensor(grad_in_t, grad_out_l1_t, scale_factor,
                                 feature_size, centre_x - 4, centre_x + 4,
                                 centre_y - 4, centre_y + 4);
      }
    }
  }
  //});
