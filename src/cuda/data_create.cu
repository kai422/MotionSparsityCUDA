 ...
 namespace
 {
         ...
         quadtree stru, float scale_factor_to_dense, int n_blocks);
     {
         ...
         if (t < input.size(0) && gh < stru.grid_height && gw < stru.grid_width)
         {
             ...
             if (!isHomogeneous(input, t, centre_x, centre_y, 8, scale_factor_to_dense))
             {
                 tree_set_bit(tree, 0);
                 //set root; bit[0]
                 int bit_idx_l1 = 1;
                 for (int hl1 = 0; hl1 < 2; ++hl1)
                 {
                     for (int wl1 = 0; wl1 < 2; ++wl1)
                     {
                         ...
                         if (!isHomogeneous(input, centre_x_l1, centre_y_l1, 4, scale_factor_to_dense))
                         {
                             ...
                             for (int hl2 = 0; hl2 < 2; ++hl2)
                             {
                                 for (int wl2 = 0; wl2 < 2; ++wl2)
                                 {
                                     ...
                                     if (!isHomogeneous(input, centre_x_l2, centre_y_l2, 2, scale_factor_to_dense))
                                     {
